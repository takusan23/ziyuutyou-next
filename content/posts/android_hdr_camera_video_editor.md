...
これはなんのためにあるのかと言うと、`10ビット`（つまり`各 RGB`は`0b00_0000_0000 から 0b11_1111_1111`、SDR なら`8ビット`なので`0b0000_0000`から`0b1111_1111`）  
...
`HLG`は、`Wikipedia`曰く途中まで`SDR`のガンマカーブとほぼ同じだそうで、`SDR`との互換性が高い。（ただ`SDR`は`Rec.2020`じゃないので完璧ってことでもない？）  
